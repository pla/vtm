...
local function material_icon_list(event)
  ...
  if event.diff then
    for item, amount in pairs(event.diff.items or {}) do
      result = result .. util.format_number(amount) .. " [item=" .. item .. "], "
      zero = zero + amount
    end
    for fluid, amount in pairs(event.diff.fluids or {}) do
      result = result .. util.format_number(amount) .. " [fluid=" .. fluid .. "], "
      zero = zero + amount
    end
  end
...
local function update_tab(gui_id)
  ...
  for _, history_data in pairs(history) do
    if (surface == "All" or surface == history_data.surface) and
