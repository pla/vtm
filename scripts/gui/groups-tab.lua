...
local function update_gui_group_detail_view(gui_data, scroll_pane, group_list)
	...
	for _, group_data in pairs(group_list) do
		...
		if
			not group_data.main_station.stock_tick
			or (group_data.main_station.stock_tick and group_data.main_station.stock_tick < game.tick - 60)
		then
		...
		refs.minimap_button.tags = flib_table.shallow_merge({
			refs.minimap_button.tags,
		...
		refs.main_station.tags = flib_table.shallow_merge({
			refs.main_station.tags,
			{ station_id = group_data.main_station.station.unit_number },
		})
		refs.editgrp_button.tags =
			flib_table.shallow_merge({ refs.editgrp_button.tags, { group_id = group_data.group_id } })

		gui_utils.slot_table_update(
			row.left_side.main_station_frame.member_stock_table,
			group_data.main_station.stock,
		...
		for _, station_data in pairs(group_data.members) do
			...
			if
				not station_data.stock_tick or (station_data.stock_tick and station_data.stock_tick < game.tick - 60)
			then
			...
			refs.member_name.tags =
				flib_table.shallow_merge({ refs.member_name.tags, { station_id = station_data.station.unit_number } })
		...
		for _, tag_data in pairs(group_data.resource_tags or {}) do
			...
			refs.member_name.tags = flib_table.shallow_merge({
				refs.member_name.tags,
				{ position = tag_data.position, surface_name = tag_data.surface.name },
			})
...
function groups_tab.update_tab(gui_data, event)
	...
	local pinned = storage.settings[player.index].groups_tab_pinned or {}
...
--- @param gui_data GuiData
--- @param event EventData|EventData.on_gui_click
function groups_tab.toggle_pin_group(gui_data, event)
	local set_name
	if event.element.tags and event.element.tags.group_set then
		set_name = event.element.tags.group_set --[[@as string]]
	else
...
function groups_tab.open_group_edit(gui_data, event)
	if not event.element.name == "new_group" and not event.element.name == "editgrp_button" then
		return
	end
