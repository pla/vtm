...
local function guess_station_type(station)
  ...
  local from_start = settings.global["vtm-p-or-r-start"].value -- TODO needs better description
...
function backend.init_stations()
...
---Register Item per station, so we can show zero items when out of stock
...
---Read items from Station circuit network(awful, maybe redo that)
---@param station_data StationData
---@param return_virtual boolean?
---@return SlotTableDef
---@return boolean --Is the limit set by circuit (true)or manual(false)
function backend.read_station_network(station_data, return_virtual)
    ...
    for _, station_data in pairs(group_data.members) do
      if station_data and station_data.station and station_data.station.valid then
        if station_data.stock_tick <= game.tick - 60 and read_stock then
          ---@type SlotTableDef
          local items = backend.read_station_network(station_data)
...
function backend.read_group_id(station)
...
function backend.get_or_create_station_data(station)
  ---@type StationData
...
function backend.update_station(station)
...
---@param mode string
function backend.update_all_stations(mode)
  if mode == "force" then
    local train_stops = game.train_manager.get_train_stops({})
    for _, station in pairs(train_stops) do
      backend.update_station(station)
    end
  elseif mode == "split" then
    -- on_tick_n for_n_of
    backend.schedule_station_refresh()
  end
  backend.clear_invalid_stations()
end
...
function backend.clear_invalid_stations()
...
--FIXME: do we still need this, rethink-redo
...
function backend.migrate_train_SE(event)
